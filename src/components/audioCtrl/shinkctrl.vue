<template>
	...
	<div class="Shrink-box">
		...
		<div class="outside font-color" @click='setCollectMusic($event)'>
			<i class="center-btn" :class='musicInfo[0].status ? "icon-collect2-list" : "icon-shrink_collect_1"'></i>
		</div>
		<div class="insert-box">
			...
			<div class="circleProgress-wrapper">
		 	...
		 	<!-- 歌曲专辑 -->
			<img :src="img" width="71" height="71" :class='musicInfo[1] ? "rotate-animate" : ""'>
	...
	export default {
		...
		methods: {
			...
			setProgress () {
	            ...
	            // 开始新的进度条
	            store.dispatch('changeShinkProgress', () => {return setInterval(() => {
	            	// 左右两边 225 -> 45
...
<style>
	...
	.insert-box {
		...
		background: #eee;
		overflow: hidden;
		box-shadow: inset 1px 1px 20px rgba(0,0,0,.8);
		-webkit-transform: rotate(-45deg);
	}
