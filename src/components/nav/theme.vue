<!-- 皮肤弹窗 -->
<template>
	<div class="theme-main animate" :style='!showSkin ? {height: 0, opacity: 0, overflow:"hidden"} : ""'>
		...
		<div class="theme-content-box">
			...
			<p class="theme-title">
				...
				@click='toggle(i)'>{{key}}</span>

				<!-- 自动切换 -->
				<i class="toggle-btn rt animate" 
				@click='toggleSwith'
				:style='{background : autoSwith ? "rgba(255,255,255,.3)" : "rgba(255,255,255,.1)"}'>
					<i class="move-circle animate" 
					:style='{left : autoSwith ? "32px" : "2.5px"}'></i>
				</i>
			...
			<div class="show-container">
				<div class="show-area animate" :style='{transform: "translateX("+ -nowBtn * 330 +"px)"}'>
					<ul class="theme-detail lf" v-for='(key, i) in skin'>
						<li class='lf' v-for='(item, j) in key'>
							...
							class="show-color"
							v-show='nowBtn === 1'
							:style='{background: item.rgb}'></div>
							<!-- 遮罩层 -->
							<b class="mask-layer animate" @click='choseskin(i, j)'></b>
							<!-- 名字显示 -->
							<p class="skin-name"><span>{{item.name}}</span></p>

							<!-- 选择的样式 -->
							<i 
							class="chose-skin" 
							v-show='i === nowSkin[1][0] && j === nowSkin[1][1]'></i>
						</li>
					</ul>
				</div>
			</div>
			...
			return {
				...
				skin	  : [
					...
					[
						{name: '炫酷黑', rgb: '#171c26'},
						{name: '深沉红', rgb:
						'-webkit-gradient(linear,left top, right bottom, from(#070000), to(#4C0001))'}, 
						{name: '主题蓝', rgb: 
						'-webkit-gradient(linear,left top, right bottom, from(#0094B9), to(#53FDD6))'},
						{name: '傲娇紫', rgb: 
						'-webkit-gradient(linear,left top, right bottom, from(#20002c), to(#cbb4d4))'}, 
						{name: '清新绿', rgb: 
						'-webkit-gradient(linear,left top, right bottom, from(#DCE35B), to(#45b649))'},
						{name: '金贵黄', rgb: 
						'-webkit-gradient(linear,left top, right bottom, from(#CAC531), to(#F3F9A7))'},
						{name: '可爱粉', rgb: 
						'-webkit-gradient(linear,left top, right bottom, from(#d9a7c7), to(#fffcdc))'},
						{name: '晨光', rgb: 
						'linear-gradient(to top, rgb(240, 194, 123), rgb(75, 18, 72))'},
						{name: '暗粉', rgb: 
						'-webkit-gradient(linear,left top, right bottom, from(#EB5757), to(#000))'}
					]
				]
			}
		},
