	...
	<div class="songDetil-total-box">
		<p class="musicList-title">
   			<span 
	   			...
	   			@click='toggle(i, key)'>{{key}}
   			</span>
   		</p>
		<div class="song-comment" v-scrollTop='scrollTop'>
			<go-up class='go-top' v-show='showGoUp' :style='{top: "calc(80% + "+ scroll +"px)"}'></go-up>
			<comment-list @total='netComment' v-show='nowshow.includes("云音乐评论")'>
			</comment-list>
			<local-comment class="comment-type-box" v-show='nowshow.includes("本地评论")'>
			</local-comment>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			scrollTop (el, windowH) {
				windowH > 200 ? this.showGoUp = true
							  : this.showGoUp = false
