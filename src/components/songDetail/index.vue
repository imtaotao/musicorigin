<template>
  <main class="animate" v-mousewheel="[exceptDom, mousewheel]">
    ...
    <!-- 歌曲信息 -->
    <div
      class="main-content-box animate-songDatil"
      @click="hide"
      id="songDetail"
    ...
    >
      <div class="music-information">
        <p class="music-name">
          <span>{{ playInfo.musicInfo[1] }}</span>
        </p>
        <ul>
          <li>
    ...
    <!-- 歌曲评论 和 相似歌曲等 -->
    <div
      class="comment-similar animate-songDatil"
      :style="{ transform: 'translateY(' + translateY + '%)' }"
    >
      <!-- 评论 -->
    ...
    matchLyr() {
      ...
      oriLrc.content.forEach((ori, i) => {
        ...
        cnLrc.content.forEach((cn, j) => {
          if (ori.time === cn.time) lyr = cn.lyr;
        });
...
.animate-songDatil {
  transition: all 0.7s cubic-bezier(0.3, 0.68, 0.64, 0.87);
  -moz-transition: all 0.7s cubic-bezier(0.3, 0.68, 0.64, 0.87);
  -webkit-transition: all 0.7s cubic-bezier(0.3, 0.68, 0.64, 0.87);
}
</style>
