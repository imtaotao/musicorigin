	...
	transition (randomStr) {
		return img => {
			...
			if (swap) {
				...
				this.toggle ? this.option.mode = 'circle'
			...
			this.removeCanvas(dom, animate[animate.length - 1]).then(_ => {
				...
				for (let i = 0; i < mount - 1; i++) {
					setTimeout(() => {
						...
						this.animate.push(new banner(dom, {img, mode, mount, speed, index: i + 1}))
					}, t * i)
				}
				...
				setTimeout(() => {
					if (randomStr !== this.randomStr) return
					this.animate.push(new banner(dom, { 
						img,
						mode, 
						mount,
						speed,
						...
						setTimeout(_ => {
							if (randomStr !== this.randomStr || !this.middleware.isanimate) return
							this.move()
						}, time)
					}))
				}, t * mount)		
