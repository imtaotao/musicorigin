...
export const util = {
  ...
  resetAlert() {
    window.alert = function (text, duration) {
      ...
      setTimeout((_) => {
        showBox.style.webkitTransition =
      ...
      var timeObj = {
        ...
        timeFormat: function () {
          this.defaultTime =
            ...
            ":" +
            this.seconds;
          this.defaultTimeBefore =
            this.year + "-" + this.month + "-" + this.date;
          this.defaultTimeBeforeZero =
            this.year +
            "-" +
            this.fizer(this.month) +
            "-" +
          ...
          if (!!t) {
            ...
            this.allzero =
              ...
              this.fizer(this.date) +
              " " +
              this.fizer(this.hours) +
              ":" +
              this.fizer(this.minutes) +
              ":" +
        ...
        returnTime: function () {
          if (!t) {
            ...
            if (!position) {
              ...
              if (position === "all") {
                !fillzero
                  ? (this.dateTime = this.all)
                  : (this.dateTime = this.allzero);
                ...
                !fillzero
                  ? (this.dateTime = this.after)
                  : (this.dateTime = this.afterzero);
              } else {
                this.dateTime = this.beforezero;
              }
            }
  ...
  verifyBrowser() {
    ...
    this.isFirefox = UA && /firefox\/\d+/.test(UA) && !this.isEdge;
    this.isSafari =
      UA && /safari\/\d+/.test(UA) && !this.isEdge && !this.isChrome;
  },

  direction(e) {
    // friefox 向上为 - 3，chorem 向上为 120
    // 现在统一上为正， 下为负
    const detail = this.isFirefox ? -e.detail : e.wheelDelta;
  ...
  onWheel(dom, except, callback) {
    const wheel = this.isFirefox ? "DOMMouseScroll" : "mousewheel";
  ...
  prompt() {
    ...
    if (this.isSafari) {
      return alert(
        "本网站不兼容Safari，为了更好的用户体验，请您用Chrome浏览器访问",
        5000
      );
    }
